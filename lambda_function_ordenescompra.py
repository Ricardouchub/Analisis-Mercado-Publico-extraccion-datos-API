...
def lambda_handler(event, context):
    ...
    NOMBRE_BUCKET = os.environ.get('S3_BUCKET_NAME')
    MI_TICKET = os.environ.get('API_TICKET')
    ...
    # Aceptamos cualquier código de estado en el rango 200-299 como exitoso.
    if 200 <= response.status_code < 300:
        datos_json = response.json()
        listado_oc = datos_json.get('Listado', [])
        
        if not listado_oc:
            print("API respondió OK, pero no retornó nuevas Órdenes de Compra. Finalizando.")
